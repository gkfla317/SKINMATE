...
def generate_recommendations(scores):
    ...
    if top_concerns_names:
        ...
        recommendation_text = f"현재 '{concerns_str}' 점수가 낮습니다.<br>"
    else:
        recommendation_text = f"'{skin_type}' 타입의 피부는 현재 특별한 관리가 필요하지 않은 좋은 상태입니다.<br>현재 루틴을 유지하세요<br>"

    if '수분' in top_concerns_names: recommendation_text += "히알루론산이 포함된 수분 크림을 사용해보세요.<br>"
    if '주름' in top_concerns_names: recommendation_text += "레티놀 또는 펩타이드 성분의 안티에이징 에센스를 추천합니다.<br>"
    if '탄력' in top_concerns_names: recommendation_text += "콜라겐이 함유된 제품으로 피부 탄력을 관리해주세요.<br>"
    # 수분 + 탄력
    if '수분' in top_concerns_names and '탄력' in top_concerns_names:
        recommendation_text += "수분과 탄력을 동시에 케어할 수 있는 멀티 기능성 앰플을 추천합니다.<br>"
    # 수분 + 주름
    if '수분' in top_concerns_names and '주름' in top_concerns_names:
        recommendation_text += "보습과 주름 개선 기능을 모두 갖춘 크림을 사용해보세요.<br>"
    # 탄력 + 주름
    if '탄력' in top_concerns_names and '주름' in top_concerns_names:
        recommendation_text += "탄력과 주름을 함께 관리할 수 있는 레티놀 함유 제품을 권장합니다.<br>"
    # 수분 + 탄력 + 주름
    if '수분' in top_concerns_names and '탄력' in top_concerns_names and '주름' in top_concerns_names:
        recommendation_text += "수분, 탄력, 주름 모두 관리가 필요합니다. 종합적인 안티에이징 솔루션을 고려해보세요.<br>"
...
def analyze_image():
    ...
    reco_data = generate_recommendations(scores)

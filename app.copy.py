...
def get_skin_scores(filepath):
    """이미지 임베딩과 ML 모델을 사용하여 피부 점수를 계산합니다."""
    try:
        import numpy as np
        import pickle
        ...
        # 4. 예측 (기본: 평균값)
        prediction = np.mean(scaled_features)
        
        # 5. 점수 정규화 (0-100 범위)
        normalized_score = max(0, min(100, (prediction + 1) * 50))
